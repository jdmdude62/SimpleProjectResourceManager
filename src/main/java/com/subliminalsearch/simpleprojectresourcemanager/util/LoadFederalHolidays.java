...
public class LoadFederalHolidays {
    ...
    public static void main(String[] args) {
        ...
        try (Connection conn = dbConfig.getDataSource().getConnection()) {
            ...
            // Get the year to load holidays for
            int year = 2025;
            if (args.length > 0) {
            ...
            // Clear existing holidays for this year (optional - comment out if you want to keep existing)
            System.out.println("Clearing existing holidays for year " + year + "...");
            clearHolidaysForYear(conn, year);
            ...
            System.out.println("\nFederal holidays loaded successfully!");
            System.out.println("Users can edit these in the Holiday Calendar to match company policies.");
    ...
    private static void clearHolidaysForYear(Connection conn, int year) throws SQLException {
        String sql = "DELETE FROM company_holidays WHERE date LIKE ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, year + "-%");
            int deleted = stmt.executeUpdate();
            System.out.println("Deleted " + deleted + " existing holidays for year " + year);
        }
    }
    
    private static void insertHoliday(Connection conn, Holiday holiday) throws SQLException {
        // Check if holiday already exists
        String checkSql = "SELECT COUNT(*) FROM company_holidays WHERE name = ? AND date = ?";
        try (PreparedStatement checkStmt = conn.prepareStatement(checkSql)) {
            checkStmt.setString(1, holiday.name);
            checkStmt.setString(2, holiday.date.toString());
            try (ResultSet rs = checkStmt.executeQuery()) {
                if (rs.next() && rs.getInt(1) > 0) {
                    System.out.println("    (Skipping - already exists: " + holiday.name + ")");
                    return;
                }
            }
        }
